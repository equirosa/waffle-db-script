...
CREATE PROCEDURE RET_CADENA_PR
	@ID INT
AS
	SELECT ID, CEDULA_DUENNIO, LOGO FROM CADENA WHERE ID=@ID;
GO
...
AS
	INSERT INTO HISTORICO_CONTRASENA VALUES(@FECHA,@CEDULA_USUARIO,@CONTRASENA);
GO
...
CREATE TABLE ORDEN(
	...
	CEDULA_USUARIO VARCHAR(50) NOT NULL FOREIGN KEY REFERENCES USUARIO(CEDULA),
	CEDULA_COMERCIO VARCHAR(50) NOT NULL FOREIGN KEY REFERENCES COMERCIO(CEDULA_JURIDICA),
	CEDULA_CONDUCTOR VARCHAR(50) NOT NULL FOREIGN KEY REFERENCES USUARIO(CEDULA),
	ESTADO VARCHAR(50) NOT NULL,
	FECHA DATE NOT NULL);
GO

CREATE PROCEDURE CRE_ORDEN_PR
	@ID_DIRECCION INT NOT NULL,
	...
	@FECHA DATE NOT NULL
AS
	INSERT INTO ORDEN(ID_DIRECCION,CEDULA_USUARIO,CEDULA_COMERCIO,CEDULA_CONDUCTOR,ESTADO,FECHA)
	VALUES(@ID_DIRECCION,@CEDULA_USUARIO,@CEDULA_COMERCIO,@CEDULA_CONDUCTOR,@ESTADO,@FECHA);
GO

CREATE PROCEDURE RET_ALL_ORDEN_PR
AS
	SELECT ID,ID_DIRECCION,CEDULA_USUARIO,CEDULA_COMERCIO,CEDULA_CONDUCTOR,ESTADO,FECHA FROM ORDEN;
GO

CREATE PROCEDURE RET_ORDEN_PR
	@ID INT,
	@ID_DIRECCION INT,
	@CEDULA_USUARIO VARCHAR(50),
	@CEDULA_COMERCIO VARCHAR(50),
...
AS
	SELECT ID,ID_DIRECCION,CEDULA_USUARIO,CEDULA_COMERCIO,CEDULA_CONDUCTOR,ESTADO,FECHA FROM ORDEN
	WHERE ID=@ID OR ID_DIRECCION=@ID_DIRECCION OR CEDULA_USUARIO=@CEDULA_USUARIO OR CEDULA_COMERCIO=@CEDULA_COMERCIO OR
		CEDULA_CONDUCTOR=@CEDULA_CONDUCTOR OR ESTADO=@ESTADO OR FECHA=@FECHA;
GO

--DETALLES DE ORDEN
CREATE TABLE DETALLE(
	ID INT IDENTITY(1,1) PRIMARY KEY,
	ID_ORDEN INT FOREIGN KEY REFERENCES ORDEN(ID),
...
CREATE PROCEDURE CRE_DETALLE_PR
	...
	@CALIFICACION INT NOT NULL
AS
	INSERT INTO DETALLE(ID_ORDEN,ID_PRODUCTO,NOMBRE_PRODUCTO,PRECIO,CANTIDAD,TOTAL,CALIFICACION)
	VALUES(@ID_ORDEN,@ID_PRODUCTO,@NOMBRE_PRODUCTO,@PRECIO,@CANTIDAD,@TOTAL,@CALIFICACION);
GO

CREATE PROCEDURE RET_ALL_DETALLE_PR
AS
	SELECT ID,ID_ORDEN,ID_PRODUCTO,NOMBRE_PRODUCTO,PRECIO,CANTIDAD,TOTAL,CALIFICACION FROM DETALLE;
GO

CREATE PROCEDURE RET_DETALLE_PR
	@ID INT NOT NULL,
	@ID_ORDEN INT NOT NULL,
	@ID_PRODUCTO INT NOT NULL,
	@CALIFICACION INT NOT NULL
AS
	SELECT ID,ID_ORDEN,ID_PRODUCTO,NOMBRE_PRODUCTO,PRECIO,CANTIDAD,TOTAL,CALIFICACION FROM DETALLE
	WHERE ID=@ID OR ID_ORDEN=@ID_ORDEN OR ID_PRODUCTO=@ID_PRODUCTO OR CALIFICACION=@CALIFICACION;
GO

CREATE PROCEDURE DEL_DETALLE_PR
	@ID INT NOT NULL
AS
	DELETE FROM DETALLE WHERE ID=@ID;
GO

CREATE PROCEDURE UPD_DETALLE_PR
	@ID INT NOT NULL,
	@ID_ORDEN INT NOT NULL,
	@ID_PRODUCTO INT NOT NULL,
	@NOMBRE_PRODUCTO VARCHAR(50) NOT NULL,
	...
	@CALIFICACION INT NOT NULL
AS
	UPDATE DETALLE SET ID_ORDEN=@ID_ORDEN,ID_PRODUCTO=@ID_PRODUCTO,NOMBRE_PRODUCTO=@NOMBRE_PRODUCTO,PRECIO=@PRECIO,CANTIDAD=@CANTIDAD,
		TOTAL=@TOTAL,CALIFICACION=@CALIFICACION WHERE ID=@ID;
GO
...
CREATE PROCEDURE CRE_EXCEPCION_PR
	@ID INT NOT NULL,
	@MENSAJE VARCHAR(500) NOT NULL
AS
	INSERT INTO EXCEPCION VALUES(@ID,@MENSAJE);
GO

CREATE PROCEDURE UPD_EXCEPTION_PR
	@ID INT NOT NULL,
	@MENSAJE VARCHAR(500) NOT NULL
AS
	UPDATE EXCEPCION SET MENSAJE=@MENSAJE WHERE ID=@ID;
GO
	--RETRIEVE ALL
...
CREATE PROCEDURE CRE_MONEDERO_PR
	...
	@SALDO FLOAT
AS
	INSERT INTO MONEDERO VALUES(@ID,@CEDULA_USUARIO,@SALDO);
GO
...
INSERT INTO ROL VALUES(1,'Usuario Final');
INSERT INTO ROL VALUES(2,'Transportista');
INSERT INTO ROL VALUES(3,'Dueno Comercio');
...
CREATE PROCEDURE CRE_USUARIOxROL_PR
	@CEDULA_USUARIO VARCHAR(50) NOT NULL,
	@ROL INT NOT NULL
AS
	INSERT INTO USUARIOxROL VALUES(@CEDULA_USUARIO,@ROL);
GO

CREATE PROCEDURE DEL_USUARIOxROL_PR
	@CEDULA_USUARIO VARCHAR(50) NOT NULL,
	@ROL INT NOT NULL
AS
	DELETE FROM USUARIOxROL WHERE CEDULA_USUARIO=@CEDULA_USUARIO AND ROL=@ROL;
GO

CREATE PROCEDURE RET_USUARIOxROL_PR
	@CEDULA_USUARIO VARCHAR(50) NOT NULL,
	@ROL INT NOT NULL
AS
	SELECT CEDULA_USUARIO, ROL FROM USUARIOxROL WHERE CEDULA_USUARIO=@CEDULA_USUARIO AND ROL=@ROL;
GO

CREATE PROCEDURE RET_ALL_USUARIOxROL_PR
AS
	SELECT CEDULA_USUARIO, ROL FROM USUARIOxROL;
GO